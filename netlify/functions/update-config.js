...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    if (!allowedGptModels.includes(data.gpt_model) ||
        !allowedWhisperServers.includes(data.whisper_server)) {
      return {
        statusCode: 400,
        body: JSON.stringify({ error: "Недопустимые значения конфигурации" })
      };
    }
    ...
    return {
      statusCode: 200,
      body: JSON.stringify({ message: "Конфигурация обновлена", config: data })
    };
  } catch (error) {
    return {
      statusCode: 500,
